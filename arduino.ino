...
void loop() {
    ...
    if (Serial.available() > 0) {
        ...
        // If the command starts with 'hentai', it's a mouse movement command
        if (command.startsWith("hentai")) {
            ...
            if (commaIndex != -1) {
                ...
                deltaX = command.substring(6, commaIndex).toInt();  // Get X-axis movement
        ...
        // If the command starts with 'loli', it's a mouse click command
        else if (command.startsWith("loli")) {
